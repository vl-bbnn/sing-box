...
func startPhysicalLinkObserver(m *platformDefaultInterfaceMonitor) (func(), error) {
	...
	receive := func() (physicalLinkDatagram, error) {
		...
		pollErr := raw.Read(func(descriptor uintptr) bool {
			...
			if receiveErr == nil {
				...
				if current != nil {
					packet.Current = &control.Interface{Index: current.Index, Name: current.Name}
				}
			}
			return true
		})
	...
	m.logger.Info("android physical link observer started diagnostic_only=true")
	reader := startPhysicalLinkReader(startedAt, receive, func() { _ = socket.Close() }, emit, func(err error) {
		m.logger.Warn("android physical link observer receive_error=", err, " diagnostic_only=true")
	})
	var closeOnce sync.Once
	return func() {
		closeOnce.Do(func() {
			reader.close()
			m.logger.Info("android physical link observer stopped diagnostic_only=true")
		})
	}, nil
}
