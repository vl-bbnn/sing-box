...
func startPhysicalLinkObserver(m *platformDefaultInterfaceMonitor) (func(), error) {
	coordinator := m.physicalUpdates.(*physicalLinkCoordinator)
	startedAt := time.Now()
	...
	receive := func() (physicalLinkDatagram, error) {
		...
		pollErr := raw.Read(func(descriptor uintptr) bool {
			...
			if receiveErr == nil {
				...
				packet.Current, packet.InterfaceRevision = coordinator.snapshot()
			}
			return true
		})
		if pollErr != nil {
	...
	emit := func(observation physicalLinkObservation) {
		forwarded := coordinator.forward(observation)
		eventSequence++
		// Diagnostics are deliberately detached from the netlink reader. A slow
		// logger must not prevent the next native loss from entering the priority
		// cancellation lane.
		sequence := eventSequence
		go func() {
			m.logger.Info(
				"android physical link observer event event_seq=", sequence,
				...
				" native_loss_forwarded=", forwarded,
				" loss_forwarding=true",
			)
		}()
	}
	...
	coordinator.attachReader(reader.stop, reader.close)
