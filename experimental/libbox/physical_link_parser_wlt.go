	...
	go func() {
		...
		for {
			...
			for _, update := range updates {
				...
				observation := observePhysicalLinkUpdate(startedAt, packet.ObservedAt, update, packet.Current)
				observation.InterfaceRevision = packet.InterfaceRevision
				emit(observation)
			}
		}
	}()
...
func (r *physicalLinkReader) stop() {
