...
const (
	...
	wltDarwinInterfaceSettle     = time.Second
...
func (s *Service) InterfaceUpdated() {
	s.interfaceUpdated(s.currentInterfaceKey(), runtime.GOOS, interfaceRecoveryGraceFor(runtime.GOOS))
}
...
func (s *Service) interfaceUpdated(currentInterfaceKey string, goos string, recoveryDelay time.Duration) {
	...
	// preventing two carrier replacements from racing each other. On Apple,
	// authoritative loss has already aborted the obsolete carrier, so only a
	// short physical-path settle delay is needed before replacing it.
...
func interfaceRecoveryGraceFor(goos string) time.Duration {
	if goos == "darwin" || goos == "ios" {
		return wltDarwinInterfaceSettle
	}
	if goos == "android" {
...
func (s *Service) beginInterfaceUpdate(currentInterfaceKey string, goos string) (*wltpkg.Carrier, string, uint64, bool) {
	...
	if s.stopped {
		return carrier, previousInterfaceKey, s.interfaceGeneration, true
	}
	if !s.initialized {
	...
	s.interfaceReady = make(chan struct{})
	s.interfaceGeneration++
	return carrier, previousInterfaceKey, s.interfaceGeneration, false
}
