...
func TestInterfaceRecoveryUsesClientReloadFallbackDelayOnAndroid(t *testing.T) {
	...
	for _, goos := range []string{"darwin", "ios"} {
		if got := interfaceRecoveryGraceFor(goos); got != time.Second {
			t.Fatalf("%s interface settle=%s, want 1s after early abort", goos, got)
		}
	}
	if got := interfaceRecoveryGraceFor("linux"); got != wltInterfaceRecoveryGrace {
		t.Fatalf("linux grace changed: %s", got)
	}
}
